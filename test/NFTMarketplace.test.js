...
contract('NFTMarketplace', (accounts) => {
  ...
  describe('start testing', async () => {
    ...
    it('create 2nd token', async () => {
      ...
      const token_id_2 = await nft_contract.getLatestId()
      assert.equal(token_id_2, 2)
      const requested_uri = await nft_contract.tokenURI(token_id_2)
      assert.equal(requested_uri,token_URI_2)
      const owner_2 = await nft_contract.ownerOf(token_id_2)
